...
namespace JC
{
    ...
    MethodRef::MethodRef(u8 tag, Stream &stream)
        :CPEntry(tag)
    {
    ...
    FieldRef::FieldRef(u8 tag, Stream &stream)
        :CPEntry(tag)
    {
        class_index = stream.read_u16();
        name_and_type_index = stream.read_u16();
    ...
    ConstantString::ConstantString(u8 tag, Stream &stream)
        :CPEntry(tag)
    {
        string_index = stream.read_u16();
    }

    ConstantClass::ConstantClass(u8 tag, Stream &stream)
        :CPEntry(tag)
    {
        name_index = stream.read_u16();
    }

    ConstantUtf8::ConstantUtf8(u8 tag, Stream &stream)
        :CPEntry(tag)
    {
